...
# Book Review Website (Sessions and Storing Data) -
...
You'll be creating a site where users can submit the review of books. By the end of this project… you should be familiar with:
...
* `GET /` - show all books and a filter form
* `GET /books/create` - show the create book form
* `POST /books/create` - process a new book
* `GET /books/update` - show the update book form
* `POST /books/update` - process an existing book
...
* once you're connected with a commandline client (mongo), start inserting documents into a database called <code>hw05</code> and a collection called <code>books</code>:
	* books will have a title, its author, its price , reader's rating, and reader's reviews
	* so to insert, just do this in the commandline client:
		<pre><code data-trim contenteditable>db.books.insert({
			...
			price: 40.0,
			rating: 3,
			review: "New C++11 Standard Fully updated."
		});
</code></pre>
	...
	* insert the following books:
		<pre><code data-trim contenteditable>
			{title: "C++ Primer", author: "Stanley B. Lippman", price: 40, rating: 4, review: "New C++11 Standard Fully updated."}
			{title: "Eloquent JavaScript", author: "Marijn Haverbeke", price: 25, rating: 4, review: "Nice price."}
</code></pre>
...
Since we're storing review of books, we'd like each document to have:
* title (a __required__ <code>String</code>)
* author  (a <code>String</code>)
* price (a <code>Number</code> that is larger than or equal to zero)
* rating (a <code>Number</code> between 0 and 5)
* review of the book (a <code>String</code>)
...
<pre><code data-trim contenteditable>mongoose.model("Books", BookSchema)</code></pre>

## Part 2 - Displaying All Book Reviews, Adding Styles, and Adding Navigation

### Overview

We'll be using mongoose to read in all of the book reviews from the database. Then, we'll be able to display the reviews in a table.
...
* in that route handler, the callback should use <code>Book.find</code> to retrieve all books!
...
* if you leave the query object empty, it'll just give back __all books__
...
* in your template, use standard <code>table</code> markup, with each row containing a book
* try opening your page in your browser to show a table of all books: `http://localhost:3000`
* finally, __add styles to your page and add some simple navigation that will appear on this page as well as any other page that you create__
	...
	* additionally, add the following navigation links (these should appear on every page):
		* retrieve books by using query parameter (`/`)
		* create a book (`/books/create`)
		* update a book (`/books/update`)
		* display added books during the user's session (`/books/mine`)
...
* make sure that the following steps from earlier in the instructions have been completed:
	* check that a link to `/books/create` has been created in all of your pages
	* ...and that you are able to parse http POST request bodies so that `req.body` is available
* create the appropriate route handlers that accepts requests for <code>/books/create</code>
* you'll create two route handlers for `/books/create`: one for showing the form and one for processing the form
...
## Part 5 - Update a Book

### Overview

In this part of the assignment, you'll create another page that contains a form to update existing books identified by their titles. The form will POST data... and then redirect back to <code>/</code>.

### Details

If there are books with the same titles, you only need to update one of them in this assignment.

* make sure that the following steps from earlier in the instructions have been completed:
	* check that a link to `/books/update` has been created in all of your pages
	* ...and that you are able to parse http POST request bodies so that `req.body` is available
* create the appropriate route handlers that accepts requests for <code>/books/update</code>
* you'll create two route handlers for `/books/update`: one for showing the form and one form processing the form
    * GET will handle rendering the form
        * create another template file
        * add a form to your template
        * create a select dropdown with options loaded with existing book titles
    * POST will handle the form submission
        * use `findOneAndUpdate` by passing `title` as a condition object and update information submitted from the form
        * your request handler that deals with POSTs will update an existing book in the database... [check out the slides](../slides/14/mongo.html)
		* when it's done, it should redirect back to the page that shows all of the books
...
## Part 7 - Book Reviews Added (by session)

Finally, create one last page, `/books/mine`,  showing all of the books that have been added by the users during their session.

* you must use the `express-session` middleware to do this ([see the relevant slides](../slides/10/sessions.html#/17))
* there are a few ways to implement this:
	* storing book objects directly in the session (as well as in the database)
	* or (more complicated) modify your schema / model so that you can store the session id of the session that created a book
		* `req.session.id` will contain the id for that particular session
* make sure you link to `/books/mine` from both of the existing pages so that the graders can see that you've implemented this feature

### Example - Show The Book Reports Added for the Session
