...
2. `readWeatherFs.js` to read in a file containing JSON data representing a list of hourly temperature and use the `analyzeTemperature` function above to create a report (Note: you can assume there are no missing values, NaN or null in the data. In other words, the data is cleaned.)
...
For this part, your homework folder structure looks like (excluding `hoffy.js`):
