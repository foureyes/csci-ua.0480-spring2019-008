...
The `temperature-resource.json` you fetched using request from given url looks like below. **Note that the request url you are using is `ROOT_URL + /temperature-resource.json` **. After the dataset return from remote, you can use the `next` property to construct next url then fetch the following dataset. The description of properties in this data is listed below.
