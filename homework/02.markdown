...
## Part 2 - Weather Analyst

In this part, you'll work with a dataset of Hourly weather data for 30 US & Canadian Cities + 6 Israeli Cities.
...
2. `readWeather.js` to read in a JSON data representing a list of hourly temperature and use the `analyzeTemperature` function above to create a report (Note: you can assume there are no missing values, NaN or null in the data. In other words, the data is cleaned.)
...
* Start by reading in the file using `fs.readFile` (you can use the absolute path)
...
```javascript=
// Format
At ${time}, the temperature in NY is ${degree in Fahrenheit}
...
* void. the analysis strings are output to `stdout`. (Use `console.log()` properly)
...
3. `map`
4. `reduce`
5. There will be a small penalty for each one not used (-2).
6. Note: try not to modify the input data during executing your function. Usually the input should be read-only and if you would like to overwrite it, creating an copy would be fine. (In this assignment, the size of data is relatively small)
...
Format (All floating point numbers are required to be rounded to two digits after decimal point)
```
...
Example
```
Head 10 lines of Temperature in NY:
At 2012-10-01 13:00:00, the temperature in NY is 59.13 (F)
...
```
Note: the regular for loop is fine, but try to use higher order functions
...
2. The date string in the dataset is represented as `YYYY-MM-dd HH:mm:ss`. This is compared based on string instead of date. So **parsing** this date string to fetch `year`, `month`, `day`, `hour`, `minute`, and `second` to instantiate an date object
...
Now that you've finished your function, you can try to call it on your data from `tempAnalytic.js`

1. In report.js: require the module that you created (`tempAnalytic.js`).
...
5. You can compare your output with the example output below
...
The average temperature over spring 2013 in New York is: 50.12 (F)
...
In this part, you'll be setting up your project to work with JSON data from a url with a sequence of asynchrnous calls.
...
3. Add the module to the beginning of `readWeatherUrl.js` using `require`
...
11. Note: you can imagine that there will be a sequence of callbacks and you need to deal with different variable scopes. Trying not to make your code **callback hell** by designing your functions properly.
...
The average pressure over spring 2013 in New York is: 1018.24 (hpa)
