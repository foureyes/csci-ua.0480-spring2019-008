...
* some emoji take up more than one "character width", even though they're one character... so you'll have to dynamically size the grid by using the `wcwidth` function
	* for example `wcwidth('😃')` gives back 2
	* this means that the number of "normal" characters between the pipes (`|`), must be 4 (the smiling emoji is one character, but its display takes up two characters 
	so 4 spaces for empty cell or a smiling emoji with a space before and after)
* the column labels should be shown as letters underneath the grid
	* they should also be padded with
