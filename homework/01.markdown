...
Now... instead of just randomly choosing the computer's move or prompting the user for above, the moves are drawn from the `MOVE_STRING`.
...
* once the moves have played out...
...
5. After the player moves or the computer presses ENTER, use the `clear` function (mentioned above in the setup; it must be `require`d) to clear the previous image of the board and show the next image
