...
* do not count a cell as empty if it's between two occupied cells vertically (this situation should not be possible anyway!)
	* for example, in the board below, row 1 would not be counted as empty
	<pre code="board"><code data-trim contenteditable>
0 | 😄 |
1 |    |
2 | 😄 |
  |----+
...
Using the board object, `board`, and a string of moves, `s` to play out a series of moves automatically. The returned `result` object will have the following properties:

* `board` - 
* `lastPieceMoved` - 
* `error` - potentially `undefined` / not created
* `winnder` - potentially `undefined` / not created
...
// "tic" is used below...
const tic = require('./connectmoji.js');
</code></pre>
